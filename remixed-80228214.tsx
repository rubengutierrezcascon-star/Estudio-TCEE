...
async function callClaude(messages, system) {
  const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1500, system, messages }) });
...
async function callClaudePDF(b64, prompt, system) {
  const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 4000, system, messages: [{ role: "user", content: [{ type: "document", source: { type: "base64", media_type: "application/pdf", data: b64 } }, { type: "text", text: prompt }] }] }) });
  const d = await res.json();
  if (d.error) throw new Error(d.error.message);
  return d.content.map(b => b.text || '').join('');
}
...
export default function App() {
  ...
  if (!settings.onboarded) return <Onboarding topics={topics} settings={settings} onDone={(t, s) => { setTopics(t); store.set("opos:topics", t); saveSettings({ ...s, onboarded: true }); }} />;
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", minHeight: "100vh", background: BG }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "20px 24px" }}>
        {view === "home" && <Home topics={topics} settings={settings} setView={setView} />}
        ...
        {view === "plan" && <WeeklyPlan topics={topics} settings={settings} onSaveSettings={saveSettings} setTopics={setTopics} />}
      </div>
    </div>
  );
}
...
function Home({ topics, settings, setView }) {
